...
class GraphLayer(nn.Module):
    ...
    def forward(self, label_emb, adj, inputs_embeds,cross_attn_mask):

        label_emb = self.hir_attn(label_emb.squeeze(0), adj)
        ...
        if self.last:
            ...
            inputs_embeds = F.normalize(inputs_embeds)
            label_emb = F.normalize(label_emb)
            cosine_sim_matrix = torch.mm(inputs_embeds,label_emb.T)

            label_emb = cosine_sim_matrix.view(batch_size,seq_len,-1)
            return label_emb
...
class GraphEncoder(nn.Module):
    def __init__(self, config, graph=True, layer=1, data_path=None, threshold=0.6, tau=1):
        super(GraphEncoder, self).__init__()
        self.config = config
        self.tau = tau
        self.label_dict = torch.load(os.path.join(data_path, 'bert_value_dict.pt'))
        self.class_pre_file = open(os.path.join(data_path,'class_words_representations.pk'), "rb")
        self.class_repr = pickle.load(self.class_pre_file)["class_representations"]
        self.class_repr = nn.Parameter(torch.Tensor(self.class_repr).cuda(), requires_grad=True)
        ...
        if graph:
            ...
            self.hir_attn = GATConv(config.hidden_size, config.hidden_size, 8)
            ...
            adj = sp.coo_matrix((np.ones(self.edge_list.shape[0]), (self.edge_list[:, 0], self.edge_list[:, 1])),
                                shape=(num_class+1, num_class+1), dtype=np.float32)
            adj = adj + adj.T.multiply(adj.T > adj) - adj.multiply(adj.T > adj)
            adj = normalize_adj(adj + sp.eye(adj.shape[0]))
            self.adj = torch.FloatTensor(np.array(adj.todense())).cuda()


    def forward(self, inputs_embeds, attention_mask, labels, embeddings):
        label_mask = self.label_name != self.tokenizer.pad_token_id
        # full name
        #这个地方应该是label name embedding
