...
from class_utils import INTERMEDIATE_DATA_FOLDER_PATH, MODELS, tensor_to_numpy,_tokenize
import sys
sys.path.append('..')
punctuation_map = dict((ord(char), None) for char in string.punctuation)
s = nltk.stem.SnowballStemmer('english')
def sentence_cls_embedding(tokenizer,text_all,mention_all,class_dict,model):
    text_all = [text.split(' ') for text in text_all]
    words_list, sub_words_list, keys_list=_tokenize(text_all, tokenizer, 512)
    class_mention_embedding = [[] for _ in range(len(class_dict))]
    with torch.no_grad():

        for i in range(len(words_list)):
            input_ids = tokenizer.convert_tokens_to_ids(sub_words_list[i])
            input_ids=torch.tensor([input_ids]).cuda()
            mentions = mention_all[i]
            outputs = model(input_ids).last_hidden_state.squeeze(0)

            for mention in mentions:
                mention_labels = mention['labels']
                mention_embeddings = torch.mean(outputs[mention['start']:mention['end']],dim=0)
                for label in mention_labels:
                    class_mention_embedding[class_dict[label]].append(mention_embeddings.cpu().numpy())
...
def sentence_encode(tokens_id, model, layer):
    ...
    all_layer_outputs = hidden_states.last_hidden_state

    layer_embedding = tensor_to_numpy(all_layer_outputs.squeeze(0))[1: -1]
...
def main(args):
    ...
    mention_data = dataset["mentions"]
    class_dict  =dataset["class_dict"]
    if args.lm_type == 'bbu':
        data = [low_list(x) for x in data]
    ...
    counts = Counter()
    import string
    #sentence_cls_embedding(tokenizer,data,mention_data,class_dict,model)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--vocab_min_occurrence", type=int, default=1)
