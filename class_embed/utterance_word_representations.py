...
from class_utils import (INTERMEDIATE_DATA_FOLDER_PATH, MODELS,
...
def main(args):
    ...
    for cls in range(len(class_names)):
        ...
        for s in split_words:
            if s not in word_to_index.keys():
                flag=False
        if flag == True:
            ...
            class_name=tokenizer.tokenize(' '.join(split_words))
            class_name_ids = tokenizer.convert_tokens_to_ids(class_name)
            class_name_ids=torch.tensor([class_name_ids]).cuda()
            class_name_embed = model(class_name_ids).last_hidden_state.squeeze(0)

            class_name_embed = torch.mean(class_name_embed,dim=0).squeeze(0)
            class_words_representations.append([class_name_embed.cpu().detach().numpy()])
            class_repres.append(class_name_embed.cpu().detach().numpy())
