...
from data_utils import _load_shard, BertDataset, BertDataset_OOD, OODataset
...
parser.add_argument('--data', type=str, default='BBN', choices=['BBN', 'fnerd'], help='Dataset.')
...
parser.add_argument('--lamb', default=0.1, type=float, help='lambda')
parser.add_argument('--thre', default=0.000, type=float, help='Threshold for keeping tokens. Denote as gamma in the paper.')
...
parser.add_argument('--max_epoch', type=int, default=0, help='mask word num')
...
if __name__ == '__main__':
    ...
    tokenizer = BertTokenizer.from_pretrained("/home/ubuntu/DM_Group/CHEN/contrastive-htc-main (copy)/bert-base-uncased")
    ...
    model = ContrastiveModelMoco.from_pretrained('bert-base-uncased',num_labels=train_dataset.fine_grained_num,cls_loss=args.cls_loss,
                                          ...
                                          lamb=args.lamb, threshold=args.thre, tau=args.tau,
                                                 memory_bank=False,
                                                 queue_size=len(train_dataset),knn_num=25,end_k=25,positive_num=args.positive_num,
    ...
    #     os.path.join('checkpoints', args.data, 'checkpoint_best_acc_aug.pt'))
    #
    ...
    output_f = open(os.path.join('checkpoints', args.data, 'aug_output.txt'), 'a+')
    ...
    for epoch in range (args.max_epoch):
        ...
        for inputs, label,idxs,id_labels,coarse_labels,fine_grained_labels in train_pbar:
            ...
            ood_ids = ood_inputs["input_ids"],ood_attention_mask=ood_inputs["attention_mask"],ood_labels = ood_labels,
                           positive_samples = positive_samples,is_train=True,epoch=step)
            optimizer.zero_grad()
        ...
        with torch.no_grad():
            ...
            for inputs, label,idxs,id_labels,coarse_labels,fine_grained_labels in pbar:
                ...
                if logits_open_cls != None:
                    logits_open_cls = torch.cat((logits_open_cls, output['logits_open_cls']), dim=0)
                else:
                    logits_open_cls = output['logits_open_cls']
        pbar.close()
        scores = evaluate(logits, truth, label_dict,map_ids=test_dataset.map_ids,flag='eval',logits_mbs=logits_mbs,logits_opens=logits_open_cls)
        ...
        if macro_f1_id_avg > best_macro_f1_id_avg:
            ...
            save(best_macro_f1_id_avg, best_macro_f1_id_avg,
                 os.path.join('checkpoints', args.data, 'checkpoint_best_acc_base.pt'))
            early_stop_count = 0
    ...
    with torch.no_grad():
        ...
        for inputs, label,idxs,id_labels,coarse_labels,fine_grained_labels in pbar:
            ...
            if logits_open_cls !=None:
                ...
                logits_open_cls = output['logits_open_cls']
    results = evaluate(logits, truth, label_dict, map_ids=test_dataset.map_ids,train_dataloader=train_dataloader, feature_test=feature_test,model=model, flag='test',logits_mbs=logits_mbs,logits_opens=logits_open_cls)
    output_f.write("contrast=" + str(args.contrast) + '\n')
